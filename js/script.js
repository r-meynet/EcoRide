...
function affichageTrajets(trips) {
    ...
    const affichageResultat = document.getElementById("nombreResultats");
    let nombreTrajets = 0;

    // On vide le contenu existant
    container.innerHTML = "";

    // Variables relatives à l'affichage d'un trajet eco
    let classEcoCard = "";
    let classEcoBtn = "";
    let classEcoLogo = "";
    ...
    trips.forEach((trip) => {
        ...
        container.innerHTML += cardTrajet;
        nombreTrajets += 1;
    });

    if (nombreTrajets <= 1) {
        affichageResultat.innerHTML = `${nombreTrajets} résultat`;
    } else {
        affichageResultat.innerHTML = `${nombreTrajets} résultats`;
    }
